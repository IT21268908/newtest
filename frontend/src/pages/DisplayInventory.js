...
const DisplayInventory = () => {
  ...
  const fetchInventory = useCallback(async () => {
    axios
        .get("http://localhost:8022/api/inventory")
        .then(async (res) => {
  ...
  const handleDelete = async (id) => {
    try {
      await axios.delete("http://localhost:8022/api/inventory/" + id);
      setVariant("Success");
  ...
  const genReport = async () => {
    try {
      const res = await axios.get(
        "http://localhost:8022/api/inventory/reporting",
        {
          responseType: "blob", 
        }
      );
  ...
  const handleImage = (fileName) => {
    ...
    return `http://localhost:8022/api/content/${fileName}`;
  };
